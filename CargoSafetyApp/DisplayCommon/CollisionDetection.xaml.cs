...
namespace L3.Cargo.Safety.Display.Common
{
    ...
    /// </summary>
    public partial class CollisionDetection : UserControl
    {
        ...
        {
            if (name.Equals(OpcTags.COLLISION_DETECTION_ENABLE.Name))
            {
                _Dispatcher.Invoke(DispatcherPriority.Normal, new Action(delegate()
                {
                    Collision_Control.IsChecked = Convert.ToBoolean(value);
                }));
            }
        }

        private void Collision_Control_TouchDown(object sender, RoutedEventArgs e)
        {
            Collision_Control.IsChecked = !Collision_Control.IsChecked;
            Collision_Control_Click(sender, e);
        }

        private void Collision_Control_Click(object sender, RoutedEventArgs e)
        {
            if (Collision_Control.IsChecked == true)
            {
                SendRequest(OpcTags.COLLISION_DETECTION_ENABLE.Name, 1);
            }
            else
            {
                SendRequest(OpcTags.COLLISION_DETECTION_ENABLE.Name, 0);
            }
        }
