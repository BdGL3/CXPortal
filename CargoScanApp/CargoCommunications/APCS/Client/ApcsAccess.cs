...
namespace L3.Cargo.Communications.APCS.Client
{
    ...
    public class ApcsAccess : IDisposable, INotifyPropertyChanged
    {
        ...
        public int CommandIpPort
        {
            get
            {
                try
                {
                    if (/*invalid?*/ !Utilities.IPPortOK(_commandIpPort))
                        _commandIpPort = Utilities.PropertyConfigureInt(CommandIpPortKey, /*minimum*/ IPEndPoint.MinPort, /*maximum*/ IPEndPoint.MaxPort, /*default*/ CommandIpPortDftValue);
                }
                catch
                {
                    if (/*invalid?*/ !Utilities.IPPortOK(_commandIpPort))
                        _commandIpPort = CommandIpPortDftValue;
            ...
            {
                if (/*active?*/ Connected)
                    throw new Exception(new StackTrace(true).GetFrame(1).GetMethod().ReflectedType.Name + " call to ConnectionIpPort: " + ClassName + " is active and cannot accept ConnectionIpPort changes");
                try
                {
                    if (/*invalid?*/ !Utilities.IPPortOK(value))
                        throw new ArgumentOutOfRangeException(); /*catch translates*/
                    bool isChange = value != _commandIpPort;
                    _commandIpPort = value;
                    if (/*change?*/ isChange)
                        try { Logger.LogInfo("ConnectionIpPort set to " + _commandIpPort.ToString()); }
                        catch { }
                }
                catch
                {
                    try { int portValue = CommandIpPort; } /*ensure value*/
                    catch { }
                    throw new ArgumentOutOfRangeException(new StackTrace(true).GetFrame(1).GetMethod().ReflectedType.Name + " " + value.ToString() + " is not an acceptable internet protocol address; ConnectionIpAddress remains " + _commandIpPort.ToString());
                }
            }
        }
        /// <summary/>
        public const int CommandIpPortDftValue = 2730;
        /// <summary/>
            ...
            try
            {
                ...
                lock (_commandSendLock)
                    if (/*run?*/ !_monitorEnd.WaitOne(CommandMargin))
                        if (/*OK?*/ _connection.GetStream() != null)
                        {
        ...
        {
#if !DEBUG
            // Any kind of (even marginally valid) reply from the APCS is a suitable sign of life
            // indicator: reset the APCS's sign of life timer by recreating it.
            WatchDogCreate();
#endif
        ...
        private void ConnectionCreate()
        {
            try
            {
                if (/*(re)create?*/ _connection == null)
                {
                    Connected = false;
                    _connection = new TcpClient();
                }
                if (/*created?*/ _connection != null)
                    if (/*(re)connect?*/ !_connection.Connected)
                    {
                        Connected = false;
        ...
        private void ConnectionDispose()
        {
            ...
            WatchDogDispose();
#if false
            try { _connection.GetStream().Close(); }
            catch { }
            try { _connection.Client.Shutdown(SocketShutdown.Both); }
            catch { }
            try { _connection.Client.Close(); }
            catch { }
            try { _connection.Client.Dispose(); }
            catch { }
#endif
        ...
        private void ConnectionPrepare()
        {
            try
            {
                ...
                if (/*(re)ready?*/ !Connected && _connection.Connected)
                {
                    foreach (PulseWidth pulseWidthValue in Enum.GetValues(typeof(PulseWidth)))
                    {
                        float period = (float)PulsePeriod(pulseWidthValue);
                        try { PulseWidthConfigure(pulseWidthValue, period, /*relaxed*/ true); }
                        catch (Exception ex) { throw new Exception("PulseWidthConfigure(" + pulseWidthValue.ToString() + ", " + period.ToString() + ", true) failed" + "\r" + ex.ToString()); }
                    }
                    if (!SetStaticPulseFrequency(OperatingMode.NonAdaptiveMobile, PulseFrequencyDual, /*confirm*/ true))
                        throw new Exception("SetStaticPulseFrequency(OperatingMode.NonAdaptiveMobile, " + PulseFrequencyDual.ToString() + ", true) failed");
                    if (!SetCurrentPulseWidth(PulseWidth.PulseWidth1, /*confirm*/ true))
                        throw new Exception("SetCurrentPulseWidth(PulseWidth.PulseWidth1, true) failed");
                    Connected = true;
                }
            }
            catch
        ...
        public TraceLevel LogLevel
        {
            ...
            {   // If the candidate value is invalid (not a known TraceLevel) or TraceLevel.Off,
                ...
                value = (TraceLevel)Utilities.Within((double)TraceLevel.Error, (double)value, (double)TraceLevel.Verbose);
                if (/*change?*/ value != _logLevel)
                {   // This candidate value represents a change: if appropriate, log it.
                    if (/*Info or more?*/ (value >= TraceLevel.Info) || (_logLevel >= TraceLevel.Info))
        ...
        /// <summary/>
        public const TraceLevel LogLevelDftValue = TraceLevel.Info;
        private TraceLevel _logLevel = LogLevelDftValue;
            ...
            try
            {
                do
                    try
                    {
                        ...
                        {
                            ConnectionPrepare( /*does nothing if all OK*/ );
#if !DEBUG
                            if (/*configured?*/ Connected)
                                if (/*run?*/ !_monitorEnd.WaitOne(0))
                                    WatchDogCreate(); /*resets if active*/
#endif
                        }
                        if (/*ready?*/ Connected)
                        {
        ...
        private void MonitorStop()
        {
            _monitorEnd.Set();
            ConnectionDispose();
            try { _monitorThread = Threads.Dispose(_monitorThread, ref _monitorEnd); }
            catch { }
            finally { _monitorThread = null; }
            if (/*anomalies?*/ _monitorException != null)
                try { Logger.LogError(Utilities.TextTidy(_monitorException.ToString())); }
                catch { }
        }
        ...
        private void PropertyChangeDispose()
        {
        ...
        /// <summary/>
        public const double PulsePeriodMaxValue = 10000.0;
        /// <summary/>
        public const double PulsePeriodMinValue = 1.0;
        /// <summary/>
        public static bool PulsePeriodOK(uint value)
        {
            return (value >= PulsePeriodMinValue) && (value <= PulsePeriodMaxValue);
        }
        ...
        public bool ResetBoard()
        {
            ...
            try { Logger.LogInfo(MethodBase.GetCurrentMethod() + " returns true"); }
            catch { }
            return true;
        }
        ...
        {
            try { SpeedMessageEvent.Dispose(); }
            catch { }
            SpeedMessageEvent = null;
        }

        #region Watch Dog (Sign of Life) timer
        private void WatchDogCreate(int timeOut = WatchDogMaxValue)
        {
            timeOut = (int)Utilities.Within(0, timeOut, WatchDogMaxValue);
            Debug.Assert(Utilities.IsWithin(0, timeOut, WatchDogMaxValue));
            WatchDogDispose();
#if false
            if (/*run?*/ !_monitorEnd.WaitOne(0))
                if (/*set?*/ timeOut >= 1 /*ms*/)
                    _watchDogTimer = new Timer(new TimerCallback(WatchDogOnExpiration), /*don't periodically call any method*/ null, timeOut, Timeout.Infinite);
#endif
        }
        private void WatchDogDispose()
        {
            try { _watchDogTimer.Dispose(); }
            catch { }
            _watchDogTimer = null;
        }
        private void WatchDogOnExpiration(Object unused)
        {
            try { Logger.LogWarning("APCS sign of life timer expired; reconnecting..."); }
            catch { }
            ConnectionDispose();
        }
        /// <summary/>
        public const int WatchDogMaxValue = 3 * Utilities.Time10SECONDS;
